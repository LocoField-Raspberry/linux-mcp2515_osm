...
/* CAN bus driver for Microchip 2515 CAN Controller with SPI Interface
 *
 * Enable One-Shot mode for MCP2515 by LocoField
 * <cs@locofield.com>
 *
...
#define DEVICE_NAME "mcp2515_osm"
...
enum mcp251x_model {
	...
	CAN_MCP251X_MCP2515_OSM	= 0x2515,
...
static irqreturn_t mcp251x_can_ist(int irq, void *dev_id)
{
	...
	while (!priv->force_quit) {
		...
		if (intf & CANINTF_MERRF) {
			if (priv->can.ctrlmode & CANCTRL_OSM) {
				if (priv->tx_busy) {
...
static const struct of_device_id mcp251x_of_match[] = {
	{
		.compatible	= "microchip,mcp2515_osm",
		.data		= (void *)CAN_MCP251X_MCP2515_OSM,
	},
	{ }
};
...
static const struct spi_device_id mcp251x_id_table[] = {
	{
		.name		= "mcp2515_osm",
		.driver_data	= (kernel_ulong_t)CAN_MCP251X_MCP2515_OSM,
	},
	{ }
};
...
static int mcp251x_can_probe(struct spi_device *spi)
{
	...
	netdev_info(net, "MCP%x (OSM) successfully initialized.\n", priv->model);
	return 0;
...
MODULE_AUTHOR("Eunsang Ko <cs@locofield.com>");
MODULE_DESCRIPTION("Microchip 2515 CAN driver with One-Shot mode");
